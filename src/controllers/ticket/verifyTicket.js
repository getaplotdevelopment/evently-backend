...
class VerifyTicketController {
  ...
  static async verifyTicket(req, res) {
    ...
    const ticket = await finOneHelper(Ticket, {
      ticketNumber: paidEvent.ticketNo
    });
    await ticket.update(
      { isTicketVerified: true },
      {
        where: { ticketNumber: paidEvent.ticketNo },
        returning: true,
        plain: true
      }
    );
  ...
  static async unverifyTicket(req, res) {
    ...
    const ticket = await finOneHelper(Ticket, {
      ticketNumber: verifiedTicket.ticketNo
    });

    await ticket.update(
      { isTicketVerified: false },
      {
        where: { ticketNumber: verifiedTicket.ticketNo },
        returning: true,
        plain: true
      }
    );
