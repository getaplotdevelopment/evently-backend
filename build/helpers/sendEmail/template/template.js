...
const html = (title, action, body, role, urls, token) => {
  ...
  if (action === 'RESET PASSWORD') {
    if (role === 'SUPER USER') {
      url = process.env.NODE_ENV === 'production' ? `${process.env.FRONTEND_PRODUCTION_URL}admin/auth/reset-password/${token}` : `${process.env.FRONTEND_APP_URL}admin/auth/reset-password/${token}`;
    } else {
      url = `${urls}/reset-password/${token}`;
    }
  } else if (action === 'ACTIVATE YOUR ACCOUNT') {
    url = `${urls}/verify/${token}`;
  } else if (action === 'CANCEL EVENT' || 'EVENT POSTPONED' || 'EVENT PAUSED' || 'EVENT RESUMED') {
    url = '';
    action = 'learn more';
  } else {
