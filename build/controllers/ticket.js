  ...
  async createTicket(req, res) {
    ...
    for (const ticket in category[0]) {
      ...
      if (availableCategory) {
        ...
        await createTicketsByCategory(availableCategory.id, ticketsPerCategory, ticketPrice);
        const summary = {
  ...
  async getAllTicket(req, res) {
    const tickets = await Ticket.findAll({
      include: [{
        ...
        as: 'events',
        attributes: {
          exclude: ['category', 'numberDays', 'startTime', 'startDate', 'finishDate', 'eventType', 'favorited', 'favoritedCount', 'eventImage', 'currentMode', 'createdAt', 'updatedAt', 'isLiked', 'isDeleted']
        }
      }, {
  ...
  async updateTicketsByCategory(req, res) {
    ...
    if (status.length !== 0) {
      for (const ticket in status[0]) {
        ...
        if (availableCategory) {
          await updateTicketsByCategory(availableCategory.id, undefined, newStatus);
          const summary = {};
          summary.ticket = availableCategory.name;
