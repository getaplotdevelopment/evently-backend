...
class CommentExperienceController {
  ...
  async createComment(req, res) {
    ...
    const createdComment = { ...newComment.dataValues,
      user: {
  ...
  async deleteComment(req, res) {
    ...
    const [updatedRow, [updatedComment]] = await CommentExperience.update(comment, {
      where,
      returning: true
    });
    const {
    ...
    if (updatedRow === 1) {
      res.status(200).json({
        status: 200,
        message: isDeleted ? 'Comment already deleted' : 'Comment successfully deleted'
      });
    }
  }
  /**
  ...
  async reportCommentExperience(req, res) {
    ...
    const newReport = {
      entity: _reports.EXPERIENCE_COMMENT,
      designation,
      entityId,
      user
    };
...
exports.default = CommentExperienceController;