...
class ExperienceController {
  ...
  async createExperience(req, res) {
    ...
    const createdExperience = { ...newExperience.dataValues,
      user: {
  ...
  async deleteExperience(req, res) {
    ...
    const [updatedRow, [updatedExperience]] = await Experience.update(experience, {
      where,
      returning: true
    });
    const {
    ...
    if (updatedRow === 1) {
      res.status(200).json({
        status: 200,
        message: isDeleted ? 'Experience already deleted' : 'Experience successfully deleted'
      });
    }
  }
  /**
